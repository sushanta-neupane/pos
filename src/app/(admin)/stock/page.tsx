...
import { ScanDialog } from '@/app/(admin)/pos/scan-dialog';
...
export default function StockPage() {
  ...
  const visibleResults = React.useMemo(
    () => results.filter((item) => item.kind === 'variant' || !item.hasVariants),
    [results]
  );
  ...
  function pick(barcodeValue: string, name: string, kind?: 'product' | 'variant') {
    setBarcode(barcodeValue);
    setQuery('');
    setResults([]);
    toast.success(`Selected ${kind === 'variant' ? 'variant' : 'product'}: ${name}`);
  ...
  return (
    <div className="space-y-5">
      ...
      <FormCard title="Stock onboarding" description="Search product or variant, then add stock to the exact barcode you want.">
            ...
            {query.trim() ? (
              <div className="rounded-md border bg-white">
                <div className="flex items-center justify-between border-b px-3 py-2 text-xs text-muted-foreground">
                  <div>{searchLoading ? 'Searching…' : `Results (${visibleResults.length})`}</div>
                  <button
                    type="button"
                ...
                <div className="max-h-56 overflow-auto">
                  {visibleResults.map((p) => (
                    <button
                      ...
                      onClick={() => pick(p.barcode, p.name, p.kind)}
                    >
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <div className="truncate text-sm font-medium">{p.name}</div>
                          <span className="shrink-0 rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                            {p.kind === 'variant' ? 'Variant' : 'Product'}
                          </span>
                        </div>
                        {p.variantLabel ? (
                          <div className="mt-0.5 text-[11px] text-muted-foreground">
                            Option: {p.variantLabel}
                          </div>
                        ) : null}
