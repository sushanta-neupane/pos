...
export default async function DashboardPage() {
		...
		if (product.variants.length > 0) {
			return (
				...
				product.variants.reduce(
					(variantSum, variant) =>
						variantSum +
						(variant.costCents ?? product.costCents) * variant.stock,
					0,
				)
			);
		}
	...
	const stockMetrics = {
		...
		weeklyInvestment: stockEntries
			.filter((row) => row.createdAt >= weeklyStart)
			.reduce((sum, row) => sum + row.totalCostCents, 0),
		yearlyInvestment: stockEntries.reduce(
			(sum, row) => sum + row.totalCostCents,
			0,
		),
	};
	...
	const investmentCards = [
		{
			...
			value: formatCents(stockMetrics.dailyInvestment),
		},
		{
			label: "Weekly Investment",
			value: formatCents(stockMetrics.weeklyInvestment),
		},
		{
			label: "Yearly Investment",
			value: formatCents(stockMetrics.yearlyInvestment),
		},
	];
		...
		<div className="space-y-6">
			<div className="flex flex-col gap-2">
				...
				<p className="text-sm text-gray-500 dark:text-gray-400">
					Live sales analytics with stock and investment visibility.
				</p>
			</div>
								...
								{lowStock.length === 0 ? (
									...
									lowStock.map((product) => (
										<TableRow key={product.id}>
											...
											<TableCell className="text-right text-error-600">
												{product.stock}
											</TableCell>
