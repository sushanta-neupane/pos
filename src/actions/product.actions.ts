"use server";

import { revalidatePath } from "next/cache";
import { z } from "zod";
import { generateBarcode } from "@/lib/barcode";
import { prisma } from "@/lib/prisma";

function getStockEntryDelegate(db: unknown) {
	const candidate = db as {
		stockEntry?: {
			create: (args: unknown) => Promise<unknown>;
			createMany: (args: unknown) => Promise<unknown>;
		};
	};
	return candidate.stockEntry;
}

const createSchema = z.object({
	name: z.string().min(1),
	barcode: z.string().trim().min(1).optional(),
	priceCents: z.number().int().nonnegative(),
	costCents: z.number().int().nonnegative(),
	stock: z.number().int().nonnegative().default(0),
	isTrending: z.boolean().default(false),
	lowStockThreshold: z.number().int().nonnegative().default(5),
	variants: z
		.array(
			z.object({
				size: z.string().trim().min(1).optional(),
				colorName: z.string().trim().min(1).optional(),
				colorHex: z
					.string()
					.trim()
					.regex(/^#([0-9a-fA-F]{6})$/, "Invalid color hex")
					.optional(),
				priceCents: z.number().int().nonnegative(),
				costCents: z.number().int().nonnegative().optional(),
				stock: z.number().int().nonnegative().default(0),
			}),
		)
		.optional(),
	// Backward compatible inputs (older UI)
	variantSizes: z.array(z.string()).optional(),
	variantColors: z.array(z.string()).optional(),
});

export async function createProduct(input: z.infer<typeof createSchema>) {
	const data = createSchema.parse(input);
	const { variants, variantSizes, variantColors, barcode: requestedBarcode, ...productData } = data;
	const barcode = requestedBarcode?.trim() || (await generateBarcode());

	if (requestedBarcode?.trim()) {
		const [existingProduct, existingVariant] = await Promise.all([
			prisma.product.findUnique({ where: { barcode } }),
			prisma.productVariant.findUnique({ where: { barcode } }),
		]);
		if (existingProduct || existingVariant) {
			throw new Error("Product code already exists");
		}
	}

	const product = await prisma.product.create({
		data: { ...productData, barcode },
	});

	let nextVariants = variants ?? [];
	if (nextVariants.length === 0) {
		const normalize = (values?: string[]) => {
			const out: string[] = [];
			for (const v of values ?? []) {
				const trimmed = v.trim();
				if (!trimmed) continue;
				if (out.some((x) => x.toLowerCase() === trimmed.toLowerCase()))
					continue;
				out.push(trimmed);
			}
			return out;
		};

		const sizes = normalize(variantSizes);
		const colors = normalize(variantColors);
		if (sizes.length > 0 || colors.length > 0) {
			const sizeValues = sizes.length > 0 ? sizes : [""];
			const colorValues = colors.length > 0 ? colors : [""];
			nextVariants = [];
			for (const size of sizeValues) {
				for (const colorName of colorValues) {
					nextVariants.push({
						size: size || undefined,
						colorName: colorName || undefined,
						priceCents: productData.priceCents,
						stock: 0,
					});
				}
			}
		}
	}

	if (nextVariants.length === 0 && productData.stock > 0) {
		await getStockEntryDelegate(prisma)?.create({
			data: {
				productId: product.id,
				name: product.name,
				barcode: product.barcode,
				quantity: productData.stock,
				costCents: product.costCents,
				totalCostCents: product.costCents * productData.stock,
			},
		});
	}

	if (nextVariants.length > 0) {
		const rows: Array<{
			productId: string;
			size?: string;
			colorName?: string;
			colorHex?: string;
			key: string;
			barcode: string;
			priceCents: number;
			costCents?: number;
			stock: number;
		}> = [];

		const seen = new Set<string>();
		for (const v of nextVariants) {
			const size = v.size?.trim() || "";
			const colorKey = (v.colorHex ?? v.colorName ?? "").trim();
			const key = `${size.toLowerCase()}|${colorKey.toLowerCase()}`;
			if (seen.has(key)) continue;
			seen.add(key);
			rows.push({
				productId: product.id,
				size: size || undefined,
				colorName: v.colorName?.trim() || undefined,
				colorHex: v.colorHex?.trim() || undefined,
				key,
				barcode: await generateBarcode("VAR"),
				priceCents: v.priceCents,
				costCents: v.costCents ?? productData.costCents,
				stock: v.stock ?? 0,
			});
		}

		await prisma.productVariant.createMany({ data: rows });

		const stockRows = rows
			.filter((row) => row.stock > 0)
			.map((row) => ({
				productId: product.id,
				name:
					[
						product.name,
						[row.size, row.colorName ?? row.colorHex].filter(Boolean).join(" / "),
					]
						.filter(Boolean)
						.join(" "),
				barcode: row.barcode,
				quantity: row.stock,
				costCents: row.costCents ?? productData.costCents,
				totalCostCents: (row.costCents ?? productData.costCents) * row.stock,
			}));
		if (stockRows.length > 0) {
			await getStockEntryDelegate(prisma)?.createMany({ data: stockRows });
		}

		const sumStock = rows.reduce((sum, r) => sum + (r.stock ?? 0), 0);
		await prisma.product.update({
			where: { id: product.id },
			data: { stock: sumStock },
		});
	}

	revalidatePath("/products");
	return product;
}

const updateSchema = z.object({
	id: z.string().min(1),
	name: z.string().min(1),
	priceCents: z.number().int().nonnegative(),
	costCents: z.number().int().nonnegative(),
	isTrending: z.boolean(),
	lowStockThreshold: z.number().int().nonnegative(),
	variants: z
		.array(
			z.object({
				id: z.string().min(1).optional(),
				size: z.string().trim().optional(),
				colorName: z.string().trim().optional(),
				colorHex: z
					.string()
					.trim()
					.regex(/^#([0-9a-fA-F]{6})$/, "Invalid color hex")
					.optional()
					.or(z.literal("")),
				barcode: z.string().trim().min(1),
				priceCents: z.number().int().nonnegative(),
				costCents: z.number().int().nonnegative(),
				stock: z.number().int().nonnegative().default(0),
			}),
		)
		.optional(),
});

export async function updateProduct(input: z.infer<typeof updateSchema>) {
	const data = updateSchema.parse(input);
	if (data.variants?.length) {
		const seenKeys = new Set<string>();
		const seenBarcodes = new Set<string>();

		for (const variant of data.variants) {
			const size = variant.size?.trim() || "";
			const colorName = variant.colorName?.trim() || "";
			const colorHex = variant.colorHex?.trim() || "";
			const keySource = colorHex || colorName;
			const key = `${size.toLowerCase()}|${keySource.toLowerCase()}`;
			if (seenKeys.has(key)) throw new Error("Duplicate variant combination");
			seenKeys.add(key);

			const barcode = variant.barcode.trim();
			const barcodeKey = barcode.toLowerCase();
			if (seenBarcodes.has(barcodeKey)) throw new Error("Duplicate variant barcode");
			seenBarcodes.add(barcodeKey);
		}

		const variantBarcodes = [...new Set(data.variants.map((variant) => variant.barcode.trim()))];
		const [existingProducts, existingVariants] = await Promise.all([
			prisma.product.findMany({
				where: {
					barcode: { in: variantBarcodes },
					id: { not: data.id },
				},
				select: { barcode: true },
			}),
			prisma.productVariant.findMany({
				where: {
					barcode: { in: variantBarcodes },
				},
				select: { id: true, barcode: true },
			}),
		]);

		const productBarcodeConflicts = new Set(
			existingProducts.map((product) => product.barcode.toLowerCase()),
		);
		const variantBarcodeConflicts = new Map(
			existingVariants.map((variant) => [variant.barcode.toLowerCase(), variant.id]),
		);

		for (const variant of data.variants) {
			const barcode = variant.barcode.trim();
			const barcodeKey = barcode.toLowerCase();
			if (productBarcodeConflicts.has(barcodeKey)) {
				throw new Error(`Variant code already exists: ${barcode}`);
			}

			const existingVariantId = variantBarcodeConflicts.get(barcodeKey);
			if (existingVariantId && existingVariantId !== variant.id) {
				throw new Error(`Variant code already exists: ${barcode}`);
			}
		}
	}

	const product = await prisma.$transaction(
		async (db) => {
			if (data.variants?.length) {
				const stockEntries: Array<{
					productId: string;
					variantId: string;
					name: string;
					barcode: string;
					quantity: number;
					costCents: number;
					totalCostCents: number;
				}> = [];

				for (const variant of data.variants) {
				const size = variant.size?.trim() || "";
				const colorName = variant.colorName?.trim() || "";
				const colorHex = variant.colorHex?.trim() || "";
				const keySource = colorHex || colorName;
				const normalizedKey = `${size.toLowerCase()}|${keySource.toLowerCase()}`;
				const barcode = variant.barcode.trim();

				if (variant.id) {
					await db.productVariant.update({
						where: { id: variant.id },
						data: {
							size: size || null,
							colorName: colorName || null,
							colorHex: colorHex || null,
							key: normalizedKey,
							barcode,
							priceCents: variant.priceCents,
							costCents: variant.costCents,
							stock: variant.stock ?? 0,
						},
					});
					continue;
				}

				const createdVariant = await db.productVariant.create({
					data: {
						productId: data.id,
						size: size || null,
						colorName: colorName || null,
						colorHex: colorHex || null,
						key: normalizedKey,
						barcode,
						priceCents: variant.priceCents,
						costCents: variant.costCents,
						stock: variant.stock ?? 0,
					},
				});

				if ((variant.stock ?? 0) > 0) {
					stockEntries.push({
						productId: data.id,
						variantId: createdVariant.id,
						name:
							[
								data.name,
								[size, colorName || colorHex].filter(Boolean).join(" / "),
							]
								.filter(Boolean)
								.join(" "),
						barcode,
						quantity: variant.stock ?? 0,
						costCents: variant.costCents,
						totalCostCents: variant.costCents * (variant.stock ?? 0),
					});
				}
			}

				if (stockEntries.length > 0) {
					await getStockEntryDelegate(db)?.createMany({
						data: stockEntries,
					});
				}
			}

			if (data.variants) {
				const allVariants = await db.productVariant.findMany({
					where: { productId: data.id },
					select: { stock: true },
				});
				const totalStock = allVariants.reduce((sum, variant) => sum + variant.stock, 0);
				await db.product.update({
					where: { id: data.id },
					data: { stock: totalStock },
				});
			}

			return db.product.update({
				where: { id: data.id },
				data: {
					name: data.name,
					priceCents: data.priceCents,
					costCents: data.costCents,
					isTrending: data.isTrending,
					lowStockThreshold: data.lowStockThreshold,
				},
			});
		},
		{ timeout: 15000 },
	);
	revalidatePath("/products");
	revalidatePath(`/products/${data.id}`);
	return product;
}

const deleteSchema = z.object({
	id: z.string().min(1),
});

export async function deleteProduct(input: z.infer<typeof deleteSchema>) {
	const data = deleteSchema.parse(input);

	await prisma.product.delete({
		where: { id: data.id },
	});

	revalidatePath("/products");
	revalidatePath("/dashboard");
	revalidatePath("/alerts");
	revalidatePath("/stock");
	return { id: data.id };
}

const stockSchema = z.object({
	barcode: z.string().min(1),
	add: z.number().int().positive(),
});

export async function addStock(input: z.infer<typeof stockSchema>) {
	const data = stockSchema.parse(input);
	const updated = await prisma.$transaction(async (db) => {
		const product = await db.product.findUnique({
			where: { barcode: data.barcode },
		});
		if (product) {
			await getStockEntryDelegate(db)?.create({
				data: {
					productId: product.id,
					name: product.name,
					barcode: product.barcode,
					quantity: data.add,
					costCents: product.costCents,
					totalCostCents: product.costCents * data.add,
				},
			});
			return db.product.update({
				where: { id: product.id },
				data: { stock: { increment: data.add } },
			});
		}

		const variant = await db.productVariant.findUnique({
			where: { barcode: data.barcode },
			include: { product: true },
		});
		if (!variant) throw new Error("Product not found");

		await db.productVariant.update({
			where: { id: variant.id },
			data: { stock: { increment: data.add } },
		});

		await getStockEntryDelegate(db)?.create({
			data: {
				productId: variant.productId,
				variantId: variant.id,
				name:
					[
						variant.product.name,
						[variant.size, variant.colorName ?? variant.colorHex]
							.filter(Boolean)
							.join(" / "),
					]
						.filter(Boolean)
						.join(" "),
				barcode: variant.barcode,
				quantity: data.add,
				costCents: variant.costCents ?? variant.product.costCents,
				totalCostCents:
					(variant.costCents ?? variant.product.costCents) * data.add,
			},
		});

		const all = await db.productVariant.findMany({
			where: { productId: variant.productId },
			select: { stock: true },
		});
		const sumStock = all.reduce((sum, v) => sum + (v.stock ?? 0), 0);

		return db.product.update({
			where: { id: variant.productId },
			data: { stock: sumStock },
		});
	});

	revalidatePath("/stock");
	revalidatePath("/products");
	revalidatePath("/dashboard");
	return updated;
}
