...
export async function searchProducts(query: string) {
		...
		prisma.productVariant.findMany({
			where: {
				OR: [
					...
					{ colorHex: { contains: q, mode: "insensitive" } },
					{
						product: {
							name: { contains: q, mode: "insensitive" },
						},
	...
	for (const v of variants) {
		...
		if (out.some((item) => item.barcode === v.barcode)) continue;
		out.push({
			id: v.product.id,
...
export async function completeSale(input: z.infer<typeof saleSchema>) {
	...
	const byBarcode = new Map<
		...
		| { kind: "variant"; variant: (typeof variants)[number] }
	>();
	for (const p of products)
		byBarcode.set(p.barcode, { kind: "product", product: p });
	for (const v of variants)
		byBarcode.set(v.barcode, { kind: "variant", variant: v });
	...
	const lineItems = data.items.map((i) => {
		...
		const priceCents =
			hit.kind === "product" ? hit.product.priceCents : hit.variant.priceCents;
		const costCents =
			hit.kind === "product"
				? hit.product.costCents
				: (hit.variant.costCents ?? hit.variant.product.costCents);
		const availableStock =
			hit.kind === "product" ? hit.product.stock : hit.variant.stock;

		const variantLabel =
			hit.kind === "variant"
				? [hit.variant.size, hit.variant.colorName ?? hit.variant.colorHex]
						.filter(Boolean)
						.join(" / ")
				: "";
